...
def decide_after_generation(state: GraphState) -> str:
    ...
    2. Does it answer the question? → END or re-generate
    """
...
def build_graph():
    ...
    2. GRADE_DOCUMENTS → filter relevant docs
    3a. Docs found     → GENERATE
    3b. No docs found  → FALLBACK_TO_LLM → GENERATE
    4. GENERATE → quality check → END or retry
    """
    ...
    # RETRIEVE → always grade documents
    workflow.add_edge(RETRIEVE, GRADE_DOCUMENTS)

    # GRADE_DOCUMENTS → GENERATE or FALLBACK
    ...
    # FALLBACK_TO_LLM → always generate
